...
function generateSuggestions(currentCoverage, targetCoverage) {
    const suggestions = [];
...
`;
    ...
    let rawTestCode = await (0, llm_service_1.callGroqApi)(initialPrompt, undefined, 6000);
    if (!rawTestCode) {
        return {
            status: 'error',
    ...
    let testCode = (0, codeParser_1.extractCodeFromMarkdown)(rawTestCode);
    if (language === 'Python') {
    ...
    if (runner) {
        while (iteration <= maxIterations) {
            ...
            if (iteration < maxIterations) {
                console.log(`Coverage ${currentCoverage}% < ${coverageTarget}%. Prompting LLM for additional tests...`);
...
`;
                const improvedRaw = await (0, llm_service_1.callGroqApi)(improvePrompt, undefined, 6000);
                if (improvedRaw) {
                    testCode = (0, codeParser_1.extractCodeFromMarkdown)(improvedRaw);
                    if (language === 'Python') {
                        testCode = (0, codeParser_1.fixPythonImports)(testCode, moduleName);
                    }
                }
