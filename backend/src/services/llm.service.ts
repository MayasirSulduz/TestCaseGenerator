...
function isOutputTruncated(content: string): boolean {
  ...
  if (truncatedEndings.test(lastLine)) return true;

  // Check if it ends with an incomplete function signature
  if (/def\s+\w+\s*\(.*$/.test(lastLine) && !lastLine.includes(':')) return true;
  ...
  if (opens > closes + 3) return true;  // allow small mismatch for string content
